 ...
 */
package com.kong.eos.sdk.utils
...
class AggregationTimeTest extends FlatSpec with ShouldMatchers {
  ...
  it should "return millis from a Serializable date" in new CommonValues {
    getMillisFromSerializable(dt.asInstanceOf[JSerializable]) should be(dt.getMillis)
    getMillisFromSerializable(timestamp.asInstanceOf[JSerializable]) should be(dt.getMillis)
    getMillisFromSerializable(date.asInstanceOf[JSerializable]) should be(dt.getMillis)
    getMillisFromSerializable(1L.asInstanceOf[JSerializable]) should be(1L)
    getMillisFromSerializable("1".asInstanceOf[JSerializable]) should be(1L)
  }

  it should "return millis from a Serializable dateTime" in new CommonValues {
    getMillisFromDateTime(dt.asInstanceOf[JSerializable]) should be(dt.getMillis)
    getMillisFromDateTime(timestamp.asInstanceOf[JSerializable]) should be(dt.getMillis)
    getMillisFromDateTime(date.asInstanceOf[JSerializable]) should be(dt.getMillis)
  }
